...
def fetch_india_vix():
    ...
    if YF_OK:
        try:
            df = yf.Ticker("^INDIAVIX").history(period="3d", interval="1d")
...
def fetch_hist(sym):
    """Fetch 60d OHLCV from yfinance with alt-symbol fallback."""
    if not YF_OK: return sym, None
    m = WATCHLIST[sym]
    symbols = [m["yf"]] + m.get("yf_alt", [])
    for yf_sym in symbols:
        try:
            df = yf.Ticker(yf_sym).history(period="60d", interval="1d")
            if df is not None and not df.empty and len(df) >= 20:
                print(f"    ✓ hist {sym} via {yf_sym} ({len(df)} days)")
                return sym, {"closes": df["Close"].tolist(), "highs": df["High"].tolist(),
                             "lows": df["Low"].tolist(), "volumes": df["Volume"].tolist()}
        except Exception as e:
            print(f"    ✗ hist {sym} {yf_sym}: {e}")
    return sym, None
...
""")
    ...
    print(f"  TATAMOTORS    : Fixed — alt symbols tried automatically")
    print(f"  OI fallback   : yfinance options chain → volume proxy")
    ...
    print(f"  Upstox API    : {'✓ SET' if UPSTOX_KEY else '✗ Not configured (plug in when ready)'}")
    print(f"\n  Dashboard     : http://localhost:5000")
    print(f"  Switch strats : Click ⚡ ONE or 🎯 DEV button in dashboard\n")

    threading.Thread(target=run_scan,  daemon=True).start()
    threading.Thread(target=scheduler, daemon=True).start()
    threading.Timer(3, lambda: webbrowser.open("http://localhost:5000")).start()
