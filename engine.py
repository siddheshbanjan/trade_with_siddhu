...
def upstox_option_chain(sym):
    """
    Fetch full option chain from Upstox with real OI, IV, and Greeks.
    Returns same format as fetch_oi_nse for drop-in compatibility.
    """
    if not upstox_ok(): return None
    try:
        inst = UPSTOX_INSTRUMENTS.get(sym)
        if not inst: return None
        # Get nearest expiry
        exp_url = "https://api.upstox.com/v2/option/contract"
        exp_r = requests.get(exp_url, headers=upstox_headers(),
                             params={"instrument_key": inst["key"]}, timeout=10)
        if exp_r.status_code != 200: return None
        expiries = exp_r.json().get("data", [])
        if not expiries: return None
        # Sort and pick nearest expiry
        expiries_sorted = sorted(expiries, key=lambda x: x.get("expiry",""))
        nearest_expiry  = expiries_sorted[0].get("expiry", "")
        if not nearest_expiry: return None

        # Get option chain
        chain_url = "https://api.upstox.com/v2/option/chain"
        chain_r = requests.get(chain_url, headers=upstox_headers(),
                               params={"instrument_key": inst["key"],
                                       "expiry_date": nearest_expiry}, timeout=12)
        if chain_r.status_code != 200: return None
        chain = chain_r.json().get("data", [])
        if not chain: return None

        spot = float(chain[0].get("underlying_spot_price", 0)) if chain else 0
        c_oi = p_oi = 0; sd = {}
        ...
        for item in chain:
            ...
            co = float(ce.get("market_data", {}).get("oi", 0))
            po = float(pe.get("market_data", {}).get("oi", 0))
            c_oi += co; p_oi += po
            sd[k] = {"co": co, "po": po}
            diff = abs(k - spot)
            if diff < min_diff:
                min_diff = diff; atm_strike = k
                # Get ATM IV from call greeks
                iv_val = ce.get("option_greeks", {}).get("iv", 0)
        ...
        print(f"    ✓ OI {sym}: Upstox (PCR {pcr}, IV {atm_iv:.1f}%)")
        return {
...
def run_scan():
    ...
    # ── SCORE ROTATION: prevent same stocks dominating every scan ─────────────
    # Normalise scores relative to this scan's distribution
    if signals:
        avg_s = sum(s["score"] for s in signals) / len(signals)
        for s in signals:
            s["norm_score"] = round(s["score"] - avg_s, 1)
    # Freshness bonus: RSI recovering from oversold + MACD bull = fresh setup
    for s in signals:
        if 35 < s["rsi"] < 48 and s["macd_bull"]:
            s["score"] = min(100, s["score"] + 4)
    # Rotation penalty: if stock was in top 5 last scan, reduce score slightly
    prev_top5 = {s["sym"] for s in state.get("top_calls", [])[:5]}
    for s in signals:
        if s["sym"] in prev_top5 and s["score"] < 80:
            s["score"] = max(0, s["score"] - 6)
