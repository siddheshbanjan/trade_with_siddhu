...
def score_stock_ONE(sym, hist, lq, oid, weights=None):
    try:
        ...
        # Bullish anomaly: unusual up move
        if return_z > 0.5 and (above_vwap or above_e20) and rsi_ok:
            action = "BUY";  opt = "CE"
        # Bearish anomaly: unusual down move
        elif return_z < -0.5 and (not above_vwap or not above_e20):
            action = "SELL"; opt = "PE"
        # High score but direction unclear — default to dominant bias
        elif score >= 65:
            action = "BUY" if above_vwap else "SELL"
            opt    = "CE"  if above_vwap else "PE"
        elif score <= 38:
            action = "SELL"; opt = "PE"
        else:
            action = "HOLD"
