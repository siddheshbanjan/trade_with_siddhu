...
UPSTOX_INSTRUMENTS = {
    # key = used for live quotes and option chain
    # hist_key = used for historical OHLCV candles
    # For indices: NSE_INDEX| prefix
    # For stocks: NSE_EQ| with ISIN for historical, NSE_EQ| for quotes
    # For F&O option chain: use the INDEX key for indices, NSE_EQ| for stocks
    "NIFTY":      {"key": "NSE_INDEX|Nifty 50",     "hist_key": "NSE_INDEX|Nifty 50"},
    "BANKNIFTY":  {"key": "NSE_INDEX|Nifty Bank",   "hist_key": "NSE_INDEX|Nifty Bank"},
    "RELIANCE":   {"key": "NSE_EQ|INE002A01018",    "hist_key": "NSE_EQ|INE002A01018"},
    "TCS":        {"key": "NSE_EQ|INE467B01029",    "hist_key": "NSE_EQ|INE467B01029"},
    "INFY":       {"key": "NSE_EQ|INE009A01021",    "hist_key": "NSE_EQ|INE009A01021"},
    "HDFCBANK":   {"key": "NSE_EQ|INE040A01034",    "hist_key": "NSE_EQ|INE040A01034"},
    "ICICIBANK":  {"key": "NSE_EQ|INE090A01021",    "hist_key": "NSE_EQ|INE090A01021"},
    "WIPRO":      {"key": "NSE_EQ|INE075A01022",    "hist_key": "NSE_EQ|INE075A01022"},
    "ADANIENT":   {"key": "NSE_EQ|INE423A01024",    "hist_key": "NSE_EQ|INE423A01024"},
    "SBIN":       {"key": "NSE_EQ|INE062A01020",    "hist_key": "NSE_EQ|INE062A01020"},
    "AXISBANK":   {"key": "NSE_EQ|INE238A01034",    "hist_key": "NSE_EQ|INE238A01034"},
    "BAJFINANCE": {"key": "NSE_EQ|INE296A01024",    "hist_key": "NSE_EQ|INE296A01024"},
    "SUNPHARMA":  {"key": "NSE_EQ|INE044A01036",    "hist_key": "NSE_EQ|INE044A01036"},
    "MARUTI":     {"key": "NSE_EQ|INE585B01010",    "hist_key": "NSE_EQ|INE585B01010"},
    "LT":         {"key": "NSE_EQ|INE018A01030",    "hist_key": "NSE_EQ|INE018A01030"},
    "KOTAKBANK":  {"key": "NSE_EQ|INE237A01028",    "hist_key": "NSE_EQ|INE237A01028"},
}
...
def upstox_intraday(sym):
    """
    Fetch today's intraday 15-min candles from Upstox.
    Returns dict with opens/highs/lows/closes/volumes or None.
    This gives real intraday VWAP, RSI, and volume profile.
    """
    if not upstox_ok(): return None
    try:
        inst = UPSTOX_INSTRUMENTS.get(sym)
        if not inst: return None
        key = inst["hist_key"]
        url = f"https://api.upstox.com/v3/historical-candle/intraday/{requests.utils.quote(key, safe='')}/30minute"
...
def upstox_option_chain(sym):
    ...
    try:
        ...
        if exp_r.status_code == 401:
            print(f"  ✗ Upstox token expired — visit /upstox/login")
            return None
        if exp_r.status_code != 200:
            print(f"  ✗ Upstox OI {sym}: HTTP {exp_r.status_code} — {exp_r.text[:80]}")
            return None
...
def run_scan():
    ...
    # 2.5 — Intraday candles + relative strength vs NIFTY
    print("  [2.5] Intraday candles (30-min) + relative strength...")
    intraday = {}
    nifty_change = lqs.get("NIFTY", {}).get("change_pct", 0) or 0
    if upstox_ok() and state["market_status"] == "OPEN":
        with ThreadPoolExecutor(max_workers=6) as ex:
            futs = {ex.submit(upstox_intraday, sym): sym for sym in WATCHLIST}
        ...
        print(f"         ✓ {len(intraday)}/{len(WATCHLIST)} intraday loaded | NIFTY: {nifty_change:+.2f}%")
    else:
        print(f"         ✗ Intraday skipped (Upstox not connected or market closed)")
