...
def score_stock_ONE(sym, hist, lq, oid, weights=None):
    try:
        ...
        # ── Market closed? Use multi-day momentum for anomaly detection ───────
        # Intraday change_pct is tiny/zero after market close.
        # Instead use 3-day and 5-day returns to find stocks with real momentum.
        market_open = state.get("market_status") == "OPEN"
        if not market_open and len(c) >= 6:
            # Use 3-day return as the "change" signal when market is closed
            change_3d  = round((c[-1] - c[-4]) / c[-4] * 100, 2) if len(c) >= 4 else change_pct
            change_5d  = round((c[-1] - c[-6]) / c[-6] * 100, 2) if len(c) >= 6 else change_pct
            # Use the larger of the two to capture recent momentum
        ...
        # ── ANOMALY SCORES — how unusual is TODAY? ────────────────────────────
        # Daily returns over last 20 days
        ...
        # ── SCORE COMPONENT 1: Price anomaly (0 to W_TECH) ───────────────────
        tech = int(W_TECH * 0.5)  # neutral baseline
        if return_z > 2.5:    tech += int(W_TECH*0.5);  reasons.append(f"Exceptional move today: +{change_pct:.1f}% ({return_z:.1f}σ above normal) 🟢🟢")
        elif return_z > 1.5:  tech += int(W_TECH*0.35); reasons.append(f"Strong anomalous move: +{change_pct:.1f}% ({return_z:.1f}σ) 🟢")
        elif return_z > 0.8:  tech += int(W_TECH*0.2);  reasons.append(f"Above-average move: +{change_pct:.1f}% ({return_z:.1f}σ) 🟡")
        elif return_z > 0:    tech += int(W_TECH*0.05); reasons.append(f"Slight positive: +{change_pct:.1f}% ({return_z:.1f}σ)")
        elif return_z < -2.5: tech -= int(W_TECH*0.4);  reasons.append(f"Severe sell-off: {change_pct:.1f}% ({return_z:.1f}σ) 🔴")
        elif return_z < -1.5: tech -= int(W_TECH*0.25); reasons.append(f"Unusual down move: {change_pct:.1f}% ({return_z:.1f}σ) 🔴")
        elif return_z < -0.8: tech -= int(W_TECH*0.1);  reasons.append(f"Below-average: {change_pct:.1f}% ({return_z:.1f}σ) 🟡")
        else:                                             reasons.append(f"Normal range: {change_pct:.1f}% ({return_z:.1f}σ) — nothing special today")
        tech = max(0, min(W_TECH, tech))

        # ── SCORE COMPONENT 2: Volume anomaly (0 to W_MOM) ───────────────────
        mom = int(W_MOM * 0.5)  # neutral baseline
        if vol_z > 3:     mom += int(W_MOM*0.5);  reasons.append(f"Massive volume spike: {vr:.1f}x avg ({vol_z:.1f}σ) — institutional activity 🟢🟢")
        elif vol_z > 2:   mom += int(W_MOM*0.4);  reasons.append(f"Very high volume: {vr:.1f}x avg ({vol_z:.1f}σ) 🟢")
        elif vol_z > 1:   mom += int(W_MOM*0.25); reasons.append(f"Above-average volume: {vr:.1f}x avg ({vol_z:.1f}σ) 🟡")
        elif vol_z > 0.3: mom += int(W_MOM*0.1);  reasons.append(f"Slightly elevated volume: {vr:.1f}x avg")
        elif vol_z < -1:  mom -= int(W_MOM*0.2);  reasons.append(f"Low volume: {vr:.1f}x avg ({vol_z:.1f}σ) — weak conviction ⚠️")
        else:                                       reasons.append(f"Normal volume: {vr:.1f}x avg")

        # Confluence bonus: anomalous price + anomalous volume = strong signal
        if abs(return_z) > 1 and vol_z > 1:
            bonus = int(W_MOM * 0.2)
            mom += bonus
            reasons.append(f"Confluence: unusual price AND volume together (+{bonus} pts) 🟢")
