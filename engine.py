...
function cardHtml(s){
  ...
  const swingSection=isSwing?`
    <div class="swing-section">
      ...
      <div class="sw-row"><span class="sw-label">EMA50</span><span class="sw-val" style="color:${s.ema50_rising?'#00ffa3':'#ff3d5a'}">${s.ema50_rising?'Rising ▲':'Falling ▼'} ₹${s.ema50}</span></div>
      <div class="sw-row"><span class="sw-label">Monthly range</span><span class="sw-val" style="color:#6b8299">${s.range_pct||0}% of range</span></div>
  ...
  const twoSection=isTwo?`
    <div class="dev-section">
      <div class="dev-row"><span class="dev-label">VIX Mode</span><span class="dev-val" style="color:${s.mode==='A_VWAP'?'#ff3d5a':s.mode==='B_MOMENTUM'?'#00ffa3':'#ffb830'}">${s.mode==='A_VWAP'?'Mode A: VWAP Retest':s.mode==='B_MOMENTUM'?'Mode B: EMA Momentum':'Hybrid'}</span></div>
      <div class="dev-row"><span class="dev-label">OI Confirmed</span><span class="dev-val" style="color:${s.oi_confirmed?'#00ffa3':'#6b8299'}">${s.oi_confirmed?'✅ Yes':'⚪ No'}</span></div>
      <div class="dev-row"><span class="dev-label">EMA9 vs EMA20</span><span class="dev-val" style="color:${s.ema9_above_20?'#00ffa3':'#ff3d5a'}">${s.ema9_above_20?'EMA9 above ▲':'EMA9 below ▼'}</span></div>
      ${s.ema_cross_bull?'<div class="dev-row"><span class="dev-label">Cross Signal</span><span class="dev-val" style="color:#00ffa3">🟢 Bullish EMA Cross</span></div>':''}
      ${s.ema_cross_bear?'<div class="dev-row"><span class="dev-label">Cross Signal</span><span class="dev-val" style="color:#ff3d5a">🔴 Bearish EMA Cross</span></div>':''}
      <div class="dev-row"><span class="dev-label">T1 OI Wall</span><span class="dev-val" style="color:#ffb830">₹${s.call_wall} CE / ₹${s.put_wall} PE</span></div>
    </div>`:
  ...
  return`<div class="card ${actionClass(s.action)} fade">
    ...
    ${isSwing?swingSection:isTwo?twoSection:devSection}
...
function render(d){
  ...
  const activeSigs = (curStrategy==='DEV'   ? (d.dev_signals||[])   :
                      curStrategy==='TWO'   ? (d.two_signals||[])   :
                      curStrategy==='SWING' ? (d.swing_signals||[]) :
                      (d.signals||[]));
  const activeTop  = (curStrategy==='DEV'   ? (d.dev_signals||[]).filter(s=>s.action!=='WAIT').slice(0,10)   :
                      curStrategy==='TWO'   ? (d.two_signals||[]).filter(s=>s.action!=='WAIT').slice(0,10)   :
                      curStrategy==='SWING' ? (d.swing_signals||[]).filter(s=>s.action!=='WAIT').slice(0,10) :
                      (d.top_calls||[]));
